...
_DTypeT_co = TypeVar(
    "_DTypeT_co", bound=np.dtype[Any], default=np.dtype[Any], covariant=True
)
...
class container(Generic[_ShapeT_co, _DTypeT_co]):
    ...
    ) -> container[_ShapeT_co, np.dtype[Any]]: ...
    ...
    ) -> container[Any, np.dtype[np.integer]]: ...
    def __rlshift__(
        self, other: _ArrayLikeInt_co, /
    ) -> container[Any, np.dtype[np.integer]]: ...
    ...
    def __and__(
        self, other: _ArrayLikeInt_co, /
    ) -> container[Any, np.dtype[np.bool | np.integer]]: ...
    __rand__ = __and__
    @overload
